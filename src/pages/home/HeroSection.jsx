...
function HeroSection() {
  ...
  const videoFacadeJSX = video?.map((item) => {
    if (player === false) {
      return (
        <picture
          className="absolute top-0 inset-0 z-0 w-full h-full"
          key={item?.id}
          onClick={handleIframeChange}
        >
          <source
      ...
      return (
        <div className="absolute top-0 inset-0 z-0 w-full h-full" key={item?.id}>
          <iframe
  ...
  return (
    <section className="relative w-full mt-16 h-[50vh] sm:h-[60vh] md:h-[70vh] lg:h-[80vh] overflow-hidden">
      {videoFacadeJSX}
      <div className="absolute bottom-[10%] left-0 right-0 px-4 sm:px-8 md:left-[5%] z-[5]">
